...
def run_migration():
    ...
    try:
        ...
        with app.app_context():
            # Get database inspector and connection
            ...
            for column_name, column_type in columns_to_add.items():
                if column_name not in existing_columns:
                    connection.execute(sql_text(f"ALTER TABLE game_state ADD COLUMN {column_name} {column_type}"))
            ...
            # Create initial market history entries if table was just created
            if 'market_history' in inspector.get_table_names():
                result = connection.execute(sql_text("SELECT COUNT(*) FROM market_history"))
                count = result.scalar()
